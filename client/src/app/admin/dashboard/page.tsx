      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          <CardBody className="p-6">
            ...
            <h3 className="text-3xl font-bold text-white mb-1">{activeStudentsCount}</h3>
        ...
        <Card>
          <CardBody className="p-6">
            ...
            <h3 className="text-3xl font-bold text-white mb-1">{totalHoursLogged.toLocaleString()}</h3>
            <p className="text-xs text-green-500 flex items-center gap-1">
        ...
        <Card>
          <CardBody className="p-6">
            ...
            <h3 className="text-3xl font-bold text-white mb-1">{pendingLogsCount}</h3>
        ...
        <Card>
          <CardBody className="p-6">
            ...
            <h3 className="text-3xl font-bold text-white mb-4">{completionRate}%</h3>
      ...
      <Card>
        <CardBody className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-semibold text-white">Recent Logs Awaiting Verification</h2>
            <button className="px-4 py-2 rounded-lg text-sm font-medium text-green-500 border border-green-500/30 hover:bg-green-500/10 transition-colors cursor-pointer">
              Verify Selected
            </button>
          </div>
                ...
                {recentActionableLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-gray-800/20 transition-colors">
                    <td className="py-4 text-center">
                      <input type="checkbox" className="rounded border-gray-700 bg-[#0a0a0a] text-blue-500 focus:ring-blue-500 cursor-pointer" />
                    </td>
                    <td className="py-4 text-white font-medium">{log.name}</td>
                    <td className="py-4 text-gray-400">
                      {log.date}, {log.timeIn} - {log.timeOut !== '--:--' ? log.timeOut : 'Present'}
                    </td>
                    <td className="py-4 text-gray-400">{log.duration}</td>
                    <td className="py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${
                        log.status === 'Verified' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' : 
                        log.status === 'Flagged' ? 'bg-red-500/10 text-red-500 border-red-500/20' : 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          <CardBody className="p-6 h-full flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-base font-semibold text-white">Activity Trends (Last 7 Days)</h2>
              <button className="flex items-center gap-2 text-sm text-blue-500 hover:text-blue-400 cursor-pointer">
          ...
          <CardBody className="p-6 flex flex-col h-full">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-base font-semibold text-white flex items-center gap-2">
                <AlertTriangle size={18} className="text-red-500" /> Flagged Logs
              </h2>
            ...
            <div className="space-y-6 flex-1">
              {flaggedLogs.slice(0, 2).map((log, index) => (
                <div key={log.id} className={index !== 0 ? "pt-5 border-t border-gray-800/50" : ""}>
                  <div className="flex justify-between items-start mb-1">
                    <h4 className="text-sm font-medium text-white">Attention Required</h4>
                    <span className="text-xs text-gray-500">{log.date}</span>
                  </div>
                  <p className="text-xs text-gray-400 leading-relaxed mb-3">
                    Log entry on {log.day} requires manual review. Duration: {log.duration}.
                  </p>
                  <div className="flex items-center gap-2 inline-flex bg-[#1a1a1a] rounded-md px-2 py-1.5 border border-gray-800">
                    <div className="w-4 h-4 rounded-full bg-red-900/50 flex items-center justify-center text-[8px] text-red-400 border border-red-900">
                      {log.initials}
                    </div>
                    <span className="text-xs text-gray-300">{log.name} • {log.studentId}</span>
                  </div>
                </div>
              ))}
            </div>

            <button className="w-full mt-6 py-3 border-t border-gray-800 text-sm font-medium text-blue-500 hover:text-blue-400 transition-colors cursor-pointer">
              View All Alerts
            </button>
          </CardBody>
        </Card>
